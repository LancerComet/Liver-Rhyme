...
class LiverRhyme {
  ...
  initPlayContext (playingContext, audioBuffer) {
    ...
    scriptNode.onaudioprocess = event => {
      ...
      for (let i = 0, length = leftChannelData.length; i < length; i++) {
        _volumeSum += Math.abs(leftChannelData[i]) + Math.abs(rightChannelData[i])
      }
      currentAvgVolume = _volumeSum / (leftChannelData.length * 2)

      if (currentAvgVolume >= this.peaks * .7) {
        console.log(true)
      }
...
function getPeaks (channelData, sampleRate = 22050) {
  ...
  for (let i = 0; i < channelData.length; i++) {
    ...
    let sum = 0
    for (let j = 0, length = singleChannelData.length; j < length; j++) {
      sum += Math.abs(singleChannelData[j])
    }
    avgs.push(sum / singleChannelData.length)
  }
